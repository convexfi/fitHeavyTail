...
# rmarkdown::render("vignettes/CovarianceEstimationHeavyTail.Rmd", "prettydoc::html_pretty", params = list(N_realiz = 100))
```
...
In the following, we generate multivariate heavy-tailed Student's $t$ distributed data and compare the performance of many different existing packages via `r params$N_realiz` Monte Carlo simulations in terms of estimation accurary, measured by the mean squared error (MSE), and cpu time.
...
res_all_T <- list()
#tic(sprintf("Total execution time for %d Monte-Carlo realizations:", params$N_realiz))
for(T in T_sweep) {
...
for(T in T_sweep) {
  if (!knitr_in_progress) setTxtProgressBar(pbar, it <- it + 1)
  # first, generate random heavy-tailed data sequentially for reproducibility
  #X <- lapply(1:params$N_realiz, function(idx) {rmvt(n = T, delta = mu, sigma = Sigma_scatter, df = nu)})
  X <- replicate(params$N_realiz, rmvt(n = T, delta = mu, sigma = Sigma_scatter, df = nu), simplify = FALSE)
  #X <- foreach(1:params$N_realiz) %do% rmvt(n = T, delta = mu, sigma = Sigma_scatter, df = nu)

  # then, run the estimation in parallel 
  #res <- lapply(X, eval_single)
  #res <- foreach(i = 1:params$N_realiz) %do% eval_single(X[[i]])
  ...
  #res <- foreach(i = 1:params$N_realiz) %dopar% eval_single(X[[i]])  # this requires: registerDoParallel(4); stopImplicitCluster()
...
res_all_T_molten$T_string <- factor(res_all_T_molten$T, levels = T_sweep)
...
N_methods <- length(unique(res_all_T_molten$method))
# if MSE_mean is NA, force time_mean to NA:
na_mask <- is.na(res_all_T_molten[res_all_T_molten$measure == "MSE", ]$value_mean)
res_all_T_molten[res_all_T_molten$measure == "time", ][na_mask, ]$value_mean <- NA
```
...
ggplot(res_all_T_molten[res_all_T_molten$measure == "MSE", ], aes(x = T, y = value_mean, col = method, shape = method)) +
  ...
  scale_shape_manual(values = 1:N_methods) +  # theme(legend.title = element_blank())
  scale_x_discrete(limits = seq(T_sweep[1], tail(T_sweep, 1), by = 10)) +
