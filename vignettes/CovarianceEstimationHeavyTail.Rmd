...
\underset{\boldsymbol{\Sigma}}{\textsf{minimize}} & 
  ...
  \frac{N}{2}\sum\limits_{t=1}^{T}\log{\left(\bar{\mathbf{x}}_t^T\boldsymbol{\Sigma}^{-1}\bar{\mathbf{x}}_t\right)}\\
  & \qquad \color{darkred}{+ \;\alpha \left(N\log\left(\textsf{Tr}\left(\boldsymbol{\Sigma}^{-1}\mathbf{T}\right)\right) + \log\det(\boldsymbol{\Sigma})\right)}
\end{array}
$$
...
& \qquad \color{darkred}{+\;\alpha \left(N\log\left(\textsf{Tr}\left(\boldsymbol{\Sigma}^{-1}\mathbf{T}\right)\right) + \log\det(\boldsymbol{\Sigma})\right) + \gamma \log{\left(1 + (\boldsymbol{\mu} - \mathbf{t})^T\boldsymbol{\Sigma}^{-1}(\boldsymbol{\mu} - \mathbf{t})\right)}}
\end{array}
$$
where $\mathbf{t}$ and $\mathbf{T}$ are the targets for  $\boldsymbol{\mu}$ and $\boldsymbol{\Sigma}$, respectively.
This leads to the following (accelerated) iteration step [@SunBabPal2015, Alg. 4]:
$$
...
\left\{
\frac{N+1}{T+2\alpha}\sum\limits_{t=1}^{T}w_t\left(\boldsymbol{\mu}_k,\boldsymbol{\Sigma}_k\right)\left(\mathbf{x}_t - \boldsymbol{\mu}_{k+1}\right)\left(\mathbf{x}_t - \boldsymbol{\mu}_{k+1}\right)^T \\\hspace{5cm}+
\frac{2\gamma}{T+2\alpha}w_\textsf{tgt}\left(\boldsymbol{\mu}_k,\boldsymbol{\Sigma}_k\right)\left(\mathbf{t} - \boldsymbol{\mu}_{k+1}\right)\left(\mathbf{t} - \boldsymbol{\mu}_{k+1}\right)^T \\\hspace{1cm}+
\frac{2\alpha}{T+2\alpha}\frac{N}{\textsf{Tr}\left(\boldsymbol{\Sigma}_k^{-1}\mathbf{T}\right)}\mathbf{T}
\right\}
$$
where
$$
...
\end{aligned}
$$
The iterations converge to the solution if and only if the conditions in Corollary 3 in [@SunBabPal2015] are satisfied.
...
- The function `fitHeavyTail::fit_mvt()` assumes the the data follows a multivariate Student's $t$ distribution and performs an MLE based on the EM algorithm [@LiuRubin95]. The formulation is
$$
\begin{array}{ll}
