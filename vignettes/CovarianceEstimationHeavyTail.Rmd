...
- The function `fitHeavyTail::fit_mvt()` assumes the the data follows a multivariate Student's $t$ distribution and performs an MLE based on the EM algorithm [@LiuRubin95]. The MLE formulation (without missing values) is
$$
\begin{array}{ll}
...
$$\textsf{kurt}_i = \frac{(T+1)(T-1)}{(T-2)(T-3)}\left(\frac{\textsf{mean}(\mathbf{x}_i^4)}{\textsf{mean}(\mathbf{x}_i^2)^2} - 3\frac{T-1}{T+1}\right).$$
