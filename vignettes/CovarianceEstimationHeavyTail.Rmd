...
The empirical distribution of daily returns from financial market variables such as exchange rates, equity prices, and interest rates, is often skewed. There are several different formulations of multivariate skewed $t$ distribution appearing in the literature [@lee2014finite] [@aas2006generalized]. The package now supports the multivariate generalized hyperbolic (GH) skewed t distribution and provides to estimate the parameters of such distribution. It is implemented in the function `fitHeavyTail::fit_mst()`. Below is a simple example to illustrate its usage:
...
- The function `fitHeavyTail::fit_mst()` assumes the data follows a multivariate skew $t$ distribution and performs an MLE based on the EM algorithm [@aas2006generalized]. Suppose $\mathbf{x}_{t}$ is a random vector following the GH Skew t distribution, its probability density function (pdf) is decided by four parameters, which are location vector $\boldsymbol{\mu}$, scatter matrix $\boldsymbol{\Sigma}$, degrees of freedom $\nu$, and skewness vector $\boldsymbol{\gamma}$. When $\boldsymbol{\gamma} = \boldsymbol{0}$, the GH skew $t$ reduces to the Student's $t$ distribution (assumption used in `fitHeavyTail::fit_mvt()`). The expression of the GH skew $t$ pdf is rather complicated:

$$f\left(\mathbf{x}\right)=\frac{2^{1-\frac{\nu+N}{2}}}{\Gamma\left(\frac{\nu}{2}\right)\left(\pi\nu\right)^{\frac{N}{2}}\vert\boldsymbol{\Sigma}\vert^{\frac{1}{2}}}\frac{K_{\frac{\nu+N}{2}}\left(\left[\left(\nu+d\left(\mathbf{x}\right)\right)\boldsymbol{\gamma}^{T}\boldsymbol{\Sigma}^{-1}\boldsymbol{\gamma}\right]^{\frac{1}{2}}\right)\exp\left(\left(\mathbf{x}-\boldsymbol{\mu}\right)^{T}\boldsymbol{\Sigma}^{-1}\boldsymbol{\gamma}\right)}{\left[\left(\nu+d\left(\mathbf{x}\right)\right)\boldsymbol{\gamma}^{T}\boldsymbol{\Sigma}^{-1}\boldsymbol{\gamma}\right]^{-\frac{\nu+N}{4}}\left(1+\frac{d\left(\mathbf{x}\right)}{\nu}\right)^{\frac{\nu+N}{2}}},$$
where $K_{\alpha}$ is the modified Bessel function and $d\left(\mathbf{x}\right)=\left(\mathbf{x}-\boldsymbol{\mu}\right)^{T}\boldsymbol{\Sigma}^{-1}\left(\mathbf{x}-\boldsymbol{\mu}\right)$. Fortunately, $\mathbf{x}_{t}$ can be represented in a hierarchical structure:
$$\begin{aligned}
\mathbf{x}_{t}\lvert\tau & \sim\mathcal{N}\left(\boldsymbol{\mu}+\frac{1}{\tau_{t}}\boldsymbol{\gamma},\frac{1}{\tau_{t}}\boldsymbol{\Sigma}\right),\\
