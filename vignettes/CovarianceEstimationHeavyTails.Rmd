...
MSE_covRobust_cov.nnve_T <- MSE_robust_covRob_T <- MSE_robustbase_covMcd_T <- MSE_MASS_cov.mve_T <- MSE_MASS_cov.trov_T <- MSE_QRM_T <- 
  MSE_Studentt_T <- MSE_Studentt_nu_T <- MSE_Cauchy_T <- MSE_Tyler_T <- MSE_SCM_T <- NULL
time_covRobust_cov.nnve_T <- time_robust_covRob_T <- time_robustbase_covMcd_T <- time_MASS_cov.mve_T <- time_MASS_cov.trov_T <- time_QRM_T <- 
...
for(T in T_sweep) {
  ...
  MSE_covRobust_cov.nnve <- MSE_robust_covRob <- MSE_robustbase_covMcd <- MSE_MASS_cov.mve <- MSE_MASS_cov.trov <- MSE_QRM <- 
    MSE_Studentt <- MSE_Studentt_nu <- MSE_Cauchy <- MSE_Tyler <- MSE_SCM <- 0
  time_covRobust_cov.nnve <- time_robust_covRob <- time_robustbase_covMcd <- time_MASS_cov.mve <- time_MASS_cov.trov <- time_QRM <- 
  ...
  for (ii in 1:N_realiz) {
    ...
    time_Studentt_nu <- time_Studentt_nu + system.time({
      Sigma_Studentt_nu <- fit_mvt(X, nu = 6)$cov
    ...
    time_Studentt <- time_Studentt + system.time({
      Sigma_Studentt <- fit_mvt(X, nu_regcoef = 5e-2)$cov
