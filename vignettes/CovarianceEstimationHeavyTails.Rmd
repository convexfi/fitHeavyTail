...
ggplot(data.frame("MSE" = MSE_all_T["100", ], "cpu_time" = time_all_T["100", ], "method" = colnames(MSE_all_T)), 
  ...
  geom_point(size = 3) +
  geom_ellipse(aes(x0 = 0.0038, y0 = 11.6, a = 8e-3, b = 4, angle = 0), col = "black", size = 0.5) +
...
2. `fitHeavyTail::fit_mvt()` (without fixing `nu`),
3. `fitHeavyTail::fit_Tyler()` or `fitHeavyTail::fit_Cauchy()` depending on the ratio $N/T$, and
4. `MASS::cov.trob()` (with the advantage of being preinstalled with base R).
