...
[`covHeavyTail`](TBD) (or perhaps [`fitHeavyTail`](TBD)) to estimate covariance matrices for heavy-tailed distributions and compares with existing
benchmark functions from different packages.
...
- `QRM::fit.mst()` has similar performance to our `fit_mvt()` but the computational complexity is unnacceptably high. It's not an option in practice.
- `MASS::cov.trov()` has the same speed as our `fit_mvt()`, while its performance depends on the number of observations (note that it requires `nu` as a parameter).
