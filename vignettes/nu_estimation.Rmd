...
eval_single <- function(X) {
  nu <- num_iterations <- cpu_time <- list()
  
  # name                  <- "nu = true"
  # fitted                <- fit_mvt(X, nu = 4)
  # nu[name]              <- fitted$nu
  ...
  # name                  <- "nu0 = 4"
  # fitted                <- fit_mvt(X, initial = list(nu = 4))
  # nu[name]              <- fitted$nu
  # num_iterations[name]  <- fitted$num_iterations
  # cpu_time[name]        <- fitted$cpu_time
  ...
  return(list("nu"= nu, "num_iterations" = num_iterations, "cpu_time" = cpu_time))
}
...
names(res_all_T) <- T_sweep  # paste0("T = ", T_sweep)
```
...
res_all_T_molten$T      <- factor(res_all_T_molten$T, levels = names(res_all_T))
...
- If $\nu$ is not optimized, then the best method to estimate it with the method "MLE-diag-resampled", albeit with a high computational cost, followed by the method "MLE-diag".
